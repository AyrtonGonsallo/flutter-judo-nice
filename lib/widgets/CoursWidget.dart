...
class CoursItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListTile(
        title: Text("${cours.jour} à ${cours.heure.substring(0,5)}",style: TextStyle(color: Colors.blue,fontWeight: FontWeight.bold),),
        subtitle: Text("Âge : ${cours.categorieAge}"),
        onTap: (){
          print("${cours.id} ${cours.jour} ${cours.heure.substring(0,5)}");
          Navigator.push(context,
            MaterialPageRoute(builder: (context)=> CourseCheckListPage( userId: userID, courseId: cours.id,))
          );
          },
      ),
    );
